...
resource "null_resource" "schema" {
    ...
    provisioner "local-exec" {
        command = <<EOF
            ...
            cd mongodb-main
            ls -ltr
            mongo --ssl --host ${aws_docdb_cluster.docdb.endpoint} --sslCAFile /tmp/global-bundle.pem --username admin1 --password roboshop1 < catalogue.js  mongo --ssl --host ${aws_docdb_cluster.docdb.endpoint} --sslCAFile /tmp/global-bundle.pem --username admin1 --password roboshop1 < users.js 
        EOF
    }
}
